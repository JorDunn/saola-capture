...
3. **CLI verbs** — `shot [--fullscreen|--region [--geometry WxH+X+Y]|--window]`,
   ...
   `--cursor`, `--copy/--no-copy`, `--no-toast`) override `capture.kdl`
...
│   ├── config.rs               # ~/.config/saola/capture.kdl (hand-walked KDL)
...
│   └── REVIEW-v0.1.md          # Stage 17
└── contrib/aur/PKGBUILD        # Stage 16
```
...
2. Dependencies — survey before pinning, each choice recorded as a
   ...
   the SPA story, but do **not** add it yet — Stage 9 adds it with Stage 2's
   evidence in hand).
...
3. **ffmpeg encode chain**: ffmpeg is not installed — print
   ...
   working command line (Stage 10 reuses it verbatim). Also test the AV1
...
4. **Audio transport decision**: PCM piped on `pipe:3` vs ffmpeg pulling
   ...
   affects A/V sync and device selection. Decide what Stage 12 implements.
...
config schema verbatim (Stage 16's README reuses it), how surfaces get
spawned at runtime.

## Stage 4 — Screencopy backend + WebP/PNG save + clipboard
...
3. `storage.rs`: save-dir resolution (config → `~/Pictures/Captures`
   ...
   the Stage 15 library reads — keep the format documented and boring).
...
## Stage 5 — Flash + toast: the PrintScr MVP
...
5. Print for Jordan the exact niri bind lines (he edits the config himself):
   ...
   `hotkey-overlay-title`, and suggest `Mod+Shift+S` for region once Stage 6
...
## Stage 6 — Region selection overlay
...
## Stage 7 — Window capture + delayed capture
...
Read Stage 2's window-capture decision and Stage 6's overlay input model.
...
## Stage 8 — Main app window (window process)
...
1. `modules/app.rs`: the `saola-capture window` iced app — `Surface::Paper`
   ...
   cursor, format/preset, audio — audio is inert until Stage 12), a
...
Parallel-safe with Stages 6–8. Read Stage 2's ScreenCast transcript and
...
teardown ordering — Stage 10 consumes this verbatim.

## Stage 10 — EncoderSink + ffmpeg CLI: recording end-to-end
...
Read Stage 2's verified ffmpeg command lines and Stage 9's handoff.
...
3. `modules/recorder.rs`: the state machine (Idle → Starting → Recording →
...
4. Wire `record start` / `record stop` / `record toggle` end-to-end for
...
Handoff: sink trait as landed, preset argument tables, measured frame-drop
behavior, the A/V timing model prepared for Stage 12.

## Stage 11 — Recording UX: tray item, region/window recording
...
## Stage 12 — Audio capture → Opus track
...
Read Stage 2's audio-transport decision and Stage 10's timing model.
...
## Stage 13 — Annotation editor I: canvas, crop, shapes
...
Parallel-safe with 9–12. Read the style guide (tool palette styling comes
from saola-theme; terracotta is the default annotation color), the
saola-theme `CLAUDE.md` iced-0.14-gotchas section, and Stage 8's editor stub.
...
Handoff: canvas architecture, tool-state model (Stage 14 extends it),
raster-composition performance notes.

## Stage 14 — Annotation editor II: text, steps, blur/pixelate
...
Read Stage 13's tool-state model.
...
## Stage 15 — History library, color picker, GIF export
...
Read Stage 4's index format and Stage 10's preset tables.
...
## Stage 16 — CI, packaging, autostart, README
...
`README.md`: what it is, install, the full `capture.kdl` reference (Stage
3's schema verbatim), CLI examples (including the scriptable
...
## Stage 17 — Review: capture-integrity and silent-failure audit (read-only)
...
source edits; findings go in the report for Stage 18. Consider a two-agent
